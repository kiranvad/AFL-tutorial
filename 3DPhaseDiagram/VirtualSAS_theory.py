...
def _preprocess_coords(coords):
    # Add tiny amounts of random noise (0.01% of data range) to avoid perfect collinearities
    data_ranges = np.ptp(coords, axis=0)  # Peak-to-peak range for each dimension
    noise_scale = data_ranges * 0.0001  # 0.01% of range
    noise = np.random.normal(0, noise_scale, coords.shape)
    processed = coords + noise
    return processed
...
class VirtualSAS_theory(Driver):
    ...
    def trace_boundaries(self,hull_tracing_ratio=0.1,drop_phases=None,reset=True):
        ...
        for label,sds in self.boundary_dataset.groupby(label_variable):
            ...
            if self.config['ternary']:
                if comps.shape[1] != 3:
                    raise ValueError(f"Ternary mode requires 3 components, but got {comps.shape[1]}")
                xy = ternary_to_xy(comps.values[:,[2,0,1]]) #shapely uses a different coordinate system than we do
            else:
                xy = comps.values[:]
            
            xy = _preprocess_coords(xy)
            hull = alphashape.alphashape(xy, hull_tracing_ratio)
    ...
    def simple_expose(self,composition: dict):
        if self.config['old_components']:
        ...
        comps = [[composition[component] for component in components]]

        phases = self.locate(comps)
        if len(phases)==0:
            label = 'D'
        elif len(phases)==1:
            label = phases[0]
        ...
        ds = xr.Dataset()
        ds['I'] = xr.DataArray(I,dims=['q'])
        ds['I_noiseless'] = xr.DataArray(I_noiseless,dims=['q'])
        ds['dI'] = xr.DataArray(dI,dims=['q'])
        ds['q'] = I.index.values
        ds['composition'] = ('component',comps[0])
        ds['component'] = components
        ds.attrs['components'] = components
        ds.attrs['components_dim'] = 'component'
        ds.attrs['labels'] = label
        ds.attrs['labels_dim'] = 'label'
        return ds
    ...
    def plot_boundary_data(self,**kwargs):
        ...
        if self.boundary_dataset is None:
            ax.text(0.5, 0.5, 'No boundary_dataset to plot.', ha='center', va='center', transform=ax.transAxes)
        else:
