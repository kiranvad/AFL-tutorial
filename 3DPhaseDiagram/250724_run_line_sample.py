...
class TemperatureLineSampler(PipelineOp):
    ...
    def calculate(self, dataset: xr.Dataset) -> Self:
        ...
        feasible_temp_idx = np.intersect1d(idx_higher_than_next_sample, idx_lower_than_max_prob)
        # feasible_temps = grid.sel({"ds_dim":self.temperature_dim}).values[feasible_temp_idx]
        # line_sample_idx = self.longest_continuous_sequence(feasible_temps, 
        #                                                    feasible_temp_idx, 
        #                                                    delta=self.delta
        #                                                 )
        # line_samples = grid.values[line_sample_idx]
        line_samples = grid.values[feasible_temp_idx]
    ...
    def longest_continuous_sequence(self, x, idx, delta=0.15):
        sequences = []
        current_seq = [idx[0].item()]
        
        for i in range(1, len(x)):
            if x[i] - x[i - 1] <= delta:
                current_seq.append(idx[i].item())
            else:
                sequences.append(current_seq)
                current_seq = [idx[i].item()]
        
        sequences.append(current_seq)
        x_line = max(sequences, key=len)
...
for r in range(num_restarts):
    ...
    step = 0
    direc = "./results/line_samples/run_%d"%(r+1)
