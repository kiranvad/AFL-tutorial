...
class BinaryProbabilityCost(PipelineOp):
    def __init__(
        ...
        self.cost_coordinate_dim = cost_coordinate_dim
        self.dim = dim 
        self.component_dim = component_dim
    ...
    def calculate(self, dataset: xr.Dataset) -> Self:
        ...
        cost = prob.sel({self.component_dim:self.cost_coordinate_dim}).values
        # Store result
...
class AcquisitionWithCost(PipelineOp):
    ...
    def __init__(
        ...
        component_dim:str = None,
        cost_coordinate_dims:List[str] = None,
        grid_dim: str = "grid",
        output_variable: str = None,
        name: str = "AcquisitonWithCost",
    ) -> None:
        ...
        self.grid_dim = grid_dim 
        self.cost_coordinate_dims = cost_coordinate_dims 
        self.component_dim = component_dim
    ...
    def calculate(self, dataset: xr.Dataset) -> Self:
        ...
        for dim, var in zip(self.cost_variables, self.cost_coordinate_dims):
            cost = dataset[var].sel({self.component_dim: dim}).values
            acqv_cost *= (1.0-cost)

        # Store result
        output = xr.DataArray(acqv_cost, dims=self.grid_dim, coords=acqv.coords)
        self.output[self.output_variable] = output
        self.output[self.output_variable].attrs["description"] = "Acquisition with Cost"
